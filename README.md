...
При полном sync плагин **объединяет** избранное/историю с сервером и **выгружает** уже просмотренное из локального `file_view` (если удаётся сопоставить файл с TMDB). Больший прогресс не затирается меньшим.
